...
def build_level(level_data):
    ...
    return platforms, spikes, exits, player_start
...
def run_game(screen, clock, level_paths, current_level_index=0):
    ...
    platforms, spikes, exits, player_start = build_level(level_data)

    if player_start is None:
        raise ValueError("Нет точки спавна игрока (символ 'p') в карте уровня")

    player = Player(*player_start)
    all_sprites = pygame.sprite.Group(*platforms, *spikes, *exits, player)
